...
pcap = LOAD '$inputfile' using PigStorage(' ') AS (
    ...
    ip_dst:chararray, --13
   --
    tcp_sport:chararray , --14
    tcp_dport:chararray , --15
    ...
    udp_sport:chararray , --30
    udp_dport:chararray , --31
...
-- ##########################################################
pkts_per_ip_dst = FOREACH (GROUP pcap BY ip_dst) {
    distinct_ip_src = DISTINCT pcap.ip_src;
    GENERATE 
        group as ip_dst, 
        COUNT(pcap) as packets,
        COUNT(distinct_ip_src) as ip_src; -- considering that each line is a packet 
}
